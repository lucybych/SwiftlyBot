...
class Starboard(commands.Cog):
    ...
    async def get_default_emote(self, guild_id: int) -> Union[discord.Emoji, str]:
        ...
        emoji = self.bot.get_emoji(default_emote)
    ...
    async def get_star_count(self, message: discord.Message) -> int:
        ...
        default_emote = await self.get_default_emote(message.guild.id)
        reactions = discord.utils.get(message.reactions, emoji=default_emote)
    ...
    async def handle_star_reaction(self, reaction: discord.Reaction, guild: discord.Guild):
        ...
        star_threshold = await find_int(guild, self.database, self.database.star_threshold)
    ...
    async def post_to_starboard(self, message: discord.Message, star_count: int):
        ...
        starboard_channel = await find_channel(message.guild, self.database, self.database.starboard_channel)
        if not starboard_channel:
            return
        starboard_collection = await self.database.get_guild_collection(message.guild.id, self.database.starboard)
        entry = await starboard_collection.find_one({ORIGINAL_MESSAGE_ID: message.id})
        if entry:
            ...
            starboard_collection.insert_one({ORIGINAL_MESSAGE_ID: message.id, STARBOARD_MESSAGE_ID: starboard_message.id, GUILD_ID: message.guild.id})
    
    async def remove_starboard_message(self, message_id: int, guild: discord.Guild):
        """Removes a post from the starboard"""
        starboard_channel = await find_channel(guild, self.database, self.database.starboard_channel)
        if not starboard_channel:
            return
        starboard_collection = await self.database.get_guild_collection(guild.id, self.database.starboard)
        entry = await starboard_collection.find_one({ORIGINAL_MESSAGE_ID: message_id})
        if not entry:
            return
        try:
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        starboard_channel = await find_channel(after.guild, self.database, self.database.starboard_channel)
        if not starboard_channel:
            return
        starboard_collection = await self.database.get_guild_collection(after.guild.id, self.database.starboard)
        entry = await starboard_collection.find_one({ORIGINAL_MESSAGE_ID: before.id})
        if entry:
            starboard_message = await starboard_channel.fetch_message(entry[STARBOARD_MESSAGE_ID])
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: Union[discord.Member, discord.User]):
        ...
        if not await find_int(user.guild, self.database, self.database.star_threshold):
            return
        if reaction.message.author == user:
            return
        if await has_valid_id(user, reaction.message.channel, reaction.message.guild, self.database, self.database.starboard_blacklist) or await has_valid_id(reaction.message.author, reaction.message.channel, reaction.message.guild, self.database, self.database.starboard_blacklist):
            return
    ...
    async def on_reaction_remove(self, reaction: discord.Reaction, user: Union[discord.Member, discord.User]):
        ...
        star_threshold = await self.database.get_config(user.guild.id, self.database.star_threshold)
        if not star_threshold or star_threshold <= 0:
            return
        if reaction.message.author == user:
            return
        if await has_valid_id(user, reaction.message.channel, reaction.message.guild, self.database, self.database.starboard_blacklist) or await has_valid_id(reaction.message.author, reaction.message.channel, reaction.message.guild, self.database, self.database.starboard_blacklist):
        ...
        default_emote = await self.get_default_emote(user.guild.id)
        if type(default_emote) == type(reaction.emoji) and reaction.emoji == default_emote:
            if sum(1 for r in reaction.message.reactions if str(r.emoji) == str(reaction.emoji)) < star_threshold:

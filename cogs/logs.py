...
class Logs(commands.Cog):
    ...
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
        ...
        for item, after_overwrite in after.overwrites.items():
            ...
            before_overwrite = before.overwrites.get(item)
    ...
    async def on_guild_emojis_update(self, guild: discord.Guild, before: Sequence[discord.Emoji], after: Sequence[discord.Emoji]):
        ...
        after_set = set(after)
        added_emojis = after_set - before_set
        for added_emoji in added_emojis:
            ...
            await server_log.send(embed=embed)
        removed_emojis = before_set - after_set
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        after_changes = [f"**Name:** {after.name}"] if after.name != before.name else []
        ...
        before_perms = await role_permissions(before)
        after_perms = await role_permissions(after)
        added_perms = []
        removed_perms = []
        ...
        sentence_a = "**Added:** " + ", ".join(perm for perm in added_perms)
        sentence_r = "**Removed: **" + ", ".join(perm for perm in removed_perms)
    ...
    async def on_guild_stickers_update(self, guild: discord.Guild, before: Sequence[discord.GuildSticker], after: Sequence[discord.GuildSticker]):
        ...
        for added_sticker in added_stickers:
            ...
            await server_log.send(embed=embed)
        removed_stickers = before_set - after_set
        ...
        for old_sticker, new_sticker in updated_stickers:
            ...
            if old_sticker.emoji != new_sticker.emoji and old_sticker.emoji.isnumeric():
                if old_sticker.emoji.isnumeric():
                    ...
                    emote_os = old_sticker.emoji
                if new_sticker.emoji.isnumeric():
                    try:
                        emote_n = await guild.fetch_emoji(int(new_sticker.emoji))
                        emote_ns = str(emote_n)
                    except Exception:
                        emote_ns = "Unknown Emote"
                else:
                    emote_ns = new_sticker.emoji
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        ...
        before_changes = [f"**Name:** {before.name}"] if before.name != after.name else []
        after_changes = [f"**Name:** {after.name}"] if after.name != before.name else []
        values = {60: "1 minute",300: "5 minutes",900: "15 minutes",1800: "30 minutes",3600: "1 hour",}
        ...
        name_b = before.afk_channel.name if before.afk_channel and before.afk_channel != after.afk_channel else "None"
        name_a = after.afk_channel.name if after.afk_channel and after.afk_channel != before.afk_channel else "None"
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if len(added_roles) > 0 and len(removed_roles) > 0:
            ...
            added_role_list = ", ".join(role for role in added_roles)
            removed_role_list = ", ".join(role for role in removed_roles)

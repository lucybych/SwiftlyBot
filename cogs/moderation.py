...
class Moderation(commands.Cog):
    ...
    async def can_unmute(self, guild: discord.Guild, user: Union[discord.Member, discord.User], timestamp: datetime) -> bool:
        ...
        moderation = await self.database.get_guild_collection(guild.id, self.database.moderation)
        mutes = moderation.find({TYPE: "mute", OFFENDER: user.id})
        async for mute in mutes:
            entry_timestamp = pytz.timezone("UTC").localize(mute[TIMESTAMP]) 
    ...
    async def check_ban_limit(self, guild: discord.Guild) -> bool:
        ...
        ban_limit = await self.database.get_config(guild.id, self.database.ban_limit)
    ...
    async def create_log_entry(self, guild: discord.Guild, mod_log: discord.TextChannel, color: discord.Color, type: str, offender: Union[discord.User,discord.Member], reason: str, responsible: discord.Member, duration: Optional[str], evidence: List[discord.Attachment] = None):
        ...
        offender_id = offender.id if isinstance(offender, Union[discord.Member, discord.User]) else 0
        ...
        log_message = await mod_log.send(embed=embed)
        if evidence:
            files = [await attachment.to_file() for attachment in evidence]
    ...
    async def generic_ban(self, ctx: commands.Context, users: List[Union[discord.Member, discord.User]], reason: str, toggle: Optional[str], evidence: List[discord.Attachment]):
    ...
    async def log_punishment(self, type: str, guild: discord.Guild, offender: Union[discord.User,discord.Member], reason: str, responsible: discord.Member, duration: Optional[str], evidence: List[discord.Attachment] = None):
        ...
        mod_log = await find_channel(guild, self.database, self.database.mod_log)
    ...
    async def perform_punishment(self, guild: discord.Guild, type: str, user: Union[discord.Member, discord.User], reason: str, until: Optional[timedelta] , role: Optional[discord.Role]) -> bool:
        ...
        if type == "ban":
            ...
            quarantine_collection = await self.database.get_guild_collection(role.guild.id, self.database.quarantine)
            try:
                user_roles = [user_role for user_role in user.roles if user_role != user.guild.default_role]
                if len(user_roles) > 0:
                    role_ids = [user_role.id for user_role in user_roles]
                else:
                    role_ids = []
                await user.add_roles(role, reason=reason)
                if len(role_ids) > 0:
            ...
            try:
                entry = await quarantine_collection.find_one({"user_id": user.id})
                await user.remove_roles(role, reason=reason)
                if entry:
    ...
    async def punishment_steps(self, guild: discord.Guild, type: str, users: List[Union[discord.Member, discord.User]], reason: str, responsible: Union[discord.Member, discord.User], duration: Optional[str], role: Optional[discord.Role], until: Optional[timedelta], evidence: List[discord.Attachment] = None) -> Tuple[List[Union[discord.Member, discord.User]], List[Union[discord.Member, discord.User]]]: 
        ...
        f_punishments = []
        for user in users:
            sf = await self.perform_punishment(guild, type, user, reason, until, role)
            if sf:
                s_punishments.append(user)
                if (type == "mute" or type == "quarantine" or type == "unmute" or type == "unquarantine" or type == "warn") and not reason in SILENT_REASONS:
    ...
    async def schedule_unmute(self, members: List[discord.Member], role: discord.Role, time_in_seconds: timedelta, reason: str, timestamp: datetime, responsible: Union[discord.User, discord.Member]):
        ...
        for member in members:
            if await self.can_unmute(member.guild, member, timestamp):
                sf = await self.perform_punishment(role.guild, "unmute", member, f"Automatic unmute from mute made {int(time_in_seconds.total_seconds())} seconds ago by {responsible.name}. (ID: {responsible.id})", None, role)
                if sf:
    ...
    async def send_offender_msg(self, server: discord.Guild, type: str, user: Union[discord.User, discord.Member], reason: str, responsible: Union[discord.Member, discord.User], duration: Optional[str]):
        ...
        if type == "mute" or type == "unmute" or type == "quarantine" or type == "unquarantine":
            ...
            return
        if duration:
            message = f"You have been {punishment} in {server.name} for {duration}. Reason: {reason}. Responsible Moderator: {responsible.name} (ID: {responsible.id}.)"
        else:
            message = f"You have been {punishment} in {server.name}. Reason: {reason}. Responsible Moderator: {responsible.name} (ID: {responsible.id})"
        try:
            await user.send(message)
        except Exception:
            pass
    
    async def set_permissions_in_channels(self, guild: discord.Guild, mute_role: discord.Role):
    ...
    async def ab(self, ctx: commands.Context, users: commands.Greedy[Union[discord.User, discord.Member]]):
    ...
    @commands.command()
    @commands.has_permissions(ban_members=True)
    async def ban(self, ctx: commands.Context, users: commands.Greedy[Union[discord.User, discord.Member]], *, reason: str = "No reason given"):
    ...
    async def bansync(self, ctx: commands.Context, file: discord.Attachment):
        ...
        users = [self.bot.get_user(int(user_id)) for user_id in user_ids if self.bot.get_user(int(user_id)) is not None]
        if len(users) <= 0:
    ...
    async def clearpunishment(self, ctx: commands.Context, users: commands.Greedy[discord.User]):
        ...
        moderation_collection = await self.database.get_guild_collection(ctx.guild.id, self.database.moderation)
        for user in users:
            results = await moderation_collection.delete_many({OFFENDER: user.id})
            num_deleted = results.deleted_count
    ...
    async def clearwarn(self, ctx: commands.Context, users: commands.Greedy[discord.User]):
        ...
        moderation_collection = await self.database.get_guild_collection(ctx.guild.id, self.database.moderation)
        for user in users:
            results = await moderation_collection.delete_many({OFFENDER: user.id, TYPE: "warn"})
            num_deleted = results.deleted_count
    ...
    async def kick(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason = "No reason provided"):
    ...
    async def modlog(self, ctx: commands.Context, users: commands.Greedy[discord.User]):
        ...
        moderation_collection = await self.database.get_guild_collection(ctx.guild.id, self.database.moderation)
        for user in users:
            entries = moderation_collection.find({OFFENDER: user.id})
            ...
            async for entry in entries:
                ...
                responsible_id = entry.get(RESPONSIBLE_MODERATOR)
                responsible = self.bot.get_user(responsible_id)
                responsible_name = responsible.name if responsible else "Unknown Moderator"
    ...
    async def mute(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, args: Optional[str]):
        """Mutes the given user(s) if possible, for the given time and reason (Indefinite if no time is provided)"""
        mute_role = await find_role(ctx.guild, self.database, self.database.mute_role)
        if not mute_role:
        ...
        if args:
            ...
            potential_time = args_list[0]
            parsed_duration = await parse_time_string(potential_time)
            if parsed_duration:
                ...
                reason = args
        if duration:
            expanded = await expand_time_string(time_val)
            time_str = "for " + expanded
        else:
            expanded = None
    ...
    @commands.has_permissions(manage_roles=True)
    async def quarantine(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason = "No reason provided"):
        """Puts user(s) into quarantine if possible, with the given reason"""
        quarantine_role = await find_role(ctx.guild, self.database, self.database.quarantine_role)
    ...
    @commands.command()
    @commands.has_permissions(ban_members=True)
    async def rban(self, ctx: commands.Context, users: commands.Greedy[Union[discord.Member, discord.User]]):
    ...
    async def reason(self, ctx: commands.Context, ids: commands.Greedy[int], *, reason: str):
        ...
        moderation_collection = await self.database.get_guild_collection(ctx.guild.id, self.database.moderation)
        for id in ids:
            entry = await moderation_collection.find_one({ID: id})
            if not entry:
    ...
    async def removepunishment(self, ctx: commands.Context, ids: commands.Greedy[int]):
        ...
        for id in ids:
            ...
            result = await moderation_collection.delete_one({ID: id})
    ...
    async def resetnick(self, ctx: commands.Context, members: commands.Greedy[discord.Member]):
    ...
    @commands.command()
    @commands.has_permissions(kick_members=True)
    async def rkick(self, ctx: commands.Context, members: commands.Greedy[discord.Member]):
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def rmute(self, ctx: commands.Context, members: commands.Greedy[discord.Member], duration: Optional[str]):
        ...
        if duration:
            ...
            if not time_in_seconds:
                ...
                return
            expanded = await expand_time_string(duration)
            time_str = "for " + expanded
        else:
    ...
    async def rtimeout(self, ctx: commands.Context, members: commands.Greedy[discord.Member], duration: str = "27d"):
    ...
    @commands.command()
    @commands.has_permissions(ban_members=True)
    async def runban(self, ctx: commands.Context, users: commands.Greedy[Union[discord.Member, discord.User]]):
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def runmute(self, ctx: commands.Context, members: commands.Greedy[discord.Member]):
    ...
    @commands.has_permissions(manage_roles=True)
    async def runquarantine(self, ctx: commands.Context, members: commands.Greedy[discord.Member]):
    ...
    @commands.command()
    @commands.has_permissions(moderate_members=True)
    async def runtimeout(self, ctx: commands.Context, members: commands.Greedy[discord.Member]):
    ...
    @commands.has_permissions(manage_nicknames=True)
    async def setnick(self, ctx: commands.Context, members: commands.Greedy[discord.Member], name="Request New Nickname"):
    ...
    @commands.command()
    @commands.has_permissions(ban_members=True)
    async def tb(self, ctx: commands.Context, users: commands.Greedy[Union[discord.Member, discord.User]]):
    ...
    @commands.command()
    @commands.has_permissions(moderate_members=True)
    async def timeout(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, args: Optional[str]):
        ...
        expanded = await expand_time_string(time_val)
        for member in members:
            self.timeout_cache[member] = ctx.author
    ...
    @commands.command()
    @commands.has_permissions(ban_members=True)
    async def uau(self, ctx: commands.Context, users: commands.Greedy[Union[discord.Member, discord.User]]):
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def unmute(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason = "No reason provided"):
    ...
    @commands.has_permissions(manage_roles=True)
    async def unquarantine(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason = "No reason provided"):
    ...
    @commands.command()
    @commands.has_permissions(moderate_members=True)
    async def untimeout(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason="No reason given"):
    ...
    async def warn(self, ctx: commands.Context, users:commands.Greedy[Union[discord.Member,discord.User]], *, reason: str = "No reason provided"):
        ...
        for user in users:
            ...
            warnings += 1
            ordinal_num = await ordinal(warnings)

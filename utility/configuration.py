...
async def update_bool(ctx: commands.Context, database: Database, type: str, sentence: str):
    ...
    configuration = await database.get_config(ctx.guild.id, type)
    if configuration:
        update = await database.update_config(ctx.guild.id, {"$set": {type: False}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully disabled {sentence}.")
        else:
            await ctx.send(f"Failed to disable {sentence}.")
    else:
        update = await database.update_config(ctx.guild.id, {"$set": {type: True}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully enabled {sentence}.")
        else:
            await ctx.send(f"Failed to enable {sentence}.")
...
async def update_id(ctx: commands.Context, database: Database, type: str, item: Union[discord.abc.GuildChannel, discord.Member, discord.User, discord.Role], sentence: str):
    ...
    if not item:
        update = await database.update_config(ctx.guild.id, {"$set": {type: 0}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully disabled the {sentence}.")
        else:
            await ctx.send(f"Failed to disable the {sentence} (Is it already disabled?).")
    else:
        update = await database.update_config(ctx.guild.id, {"$set": {type: item.id}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully set the {sentence} to {item.name}.")
        else:
            await ctx.send(f"Failed to set the {sentence} to {item.name} (Is it already set to this?).")
...
async def update_id_list(ctx: commands.Context, database: Database, type: str, item: Union[discord.Guild, discord.abc.GuildChannel, discord.Member, discord.User, discord.Role], sentence: str):
    ...
    if not item:
        update = await database.update_config(ctx.guild.id, {"$set": {type: []}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully reset the {sentence}.")
        else:
        ...
        if item.id in configuration:
            ...
            if update.modified_count > 0:
                await ctx.send(f"Successfully removed {item.name} from {sentence}.")
            else:
                await ctx.send(f"Failed to remove {item.name} from {sentence}.")
        else:
            ...
            update = await database.update_config(ctx.guild.id, {"$set": {type: configuration}})
            if update.modified_count > 0:
                await ctx.send(f"Successfully added {item.name} to {sentence}.")
            else:
                await ctx.send(f"Failed to add {item.name} to {sentence}")
...
async def update_int(ctx: commands.Context, database: Database, type: str, id: int, sentence: str):
    ...
    if not id:
        ...
        if id < 0:
            await ctx.send("Invalid number, please ensure that the number is positive.")
            return
        update = await database.update_config(ctx.guild.id, {"$set": {type: id}})
        if update.modified_count > 0:
            await ctx.send(f"Succesfully set the {sentence} to {id}.")
        else:
            await ctx.send(f"Failed to set the {sentence} to {id} (Is it already set to this number?).")
...
async def update_list(ctx: commands.Context, database: Database, type: str, item: str, sentence: str):
    ...
    if not item:
        ...
        if item in configuration:
            ...
            update = await database.update_config(ctx.guild.id, {"$set": {type: configuration}})
            if update.modified_count > 0:
                await ctx.send(f"Successfully removed {item} from {sentence}.")
            else:
                await ctx.send(f"Failed to remove {item} from {sentence}.")
        else:
            if not configuration:
                configuration = [item]
            else:
            ...
            update = await database.update_config(ctx.guild.id, {"$set": {type: configuration}})
            if update.modified_count > 0:
                await ctx.send(f"Successfully added {item} to {sentence}.")
            else:
                await ctx.send(f"Failed to add {item} to {sentence}")
...
async def update_str(ctx: commands.Context, database: Database, type: str, item: str, sentence: str):
    ...
    if not item:
        ...
        if update.modified_count > 0:
            await ctx.send(f"Successfully set the {sentence} to {item}.")
        else:
            await ctx.send(f"Failed to set the {sentence} to {item}. (Is it already set to this message?)")

async def update_time(ctx: commands.Context, database: Database, type: str, time: str, sentence: str):
    """Updates a time string in the database by either setting it to Infinite, 0s, or to the provided time"""
    if not time:
        update = await database.update_config(ctx.guild.id, {"$set": {type: "0s"}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully disabled the {sentence}.")
        else:
            await ctx.send(f"Failed to disable the {sentence} (Is it already disabled?).")
    else:
        time_string = await parse_time_string(time)
        if not time_string:
            await ctx.send("Inputted time is not valid (Make sure it is of the format <num><type>, Ex. 2d or 4w5m)")
        expanded_time = await expand_time_string(time)
        update = await database.update_config(ctx.guild.id, {"$set": {type: time}})
        if update.modified_count > 0:
            await ctx.send(f"Successfully set the {sentence} to {expanded_time}.")
        else:
            await ctx.send(f"Failed to set the {sentence} to {expanded_time} (Is it already set to this time?).")